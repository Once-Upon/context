...
import { UNISWAP_V2_ROUTERS } from 'src/helpers/constants';
import { ENJOY_CONTRACT_ADDRESS, ENJOY_ABI } from './constants';
...
export const detect = (transaction: Transaction): boolean => {
  ...
  const decoded = decodeTransactionInput(transaction.input as Hex, );

  if (!decoded) return false;

  if (
    decoded.functionName !== 'addLiquidity' &&
