...
describe('FriendTech', () => {
  it('Should detect FriendTech transaction', () => {
    const friendTech1 = detect(friendTech0xde5ce243 as unknown as Transaction);
    expect(friendTech1).toBe(true);

    const friendTech2 = detect(friendTech0xe65b4bd6 as unknown as Transaction);
    expect(friendTech2).toBe(true);

    const friendTech3 = detect(friendTech0xed2dd79e as unknown as Transaction);
    expect(friendTech3).toBe(true);

    const friendTech4 = detect(friendTech0x703647d1 as unknown as Transaction);
    expect(friendTech4).toBe(true);
  });

  it('Should not detect as FriendTech', () => {
    const friendTech1 = detect(catchall0xc35c01ac as unknown as Transaction);
    expect(friendTech1).toBe(false);
  });

  it('Should generate FriendTech context', () => {
    const friendTech1 = generate(
    ...
    expect(containsBigInt(friendTech1.context)).toBe(false);

    const friendTech2 = generate(
    ...
    expect(containsBigInt(friendTech2.context)).toBe(false);

    const friendTech3 = generate(
    ...
    expect(containsBigInt(friendTech3.context)).toBe(false);

    const friendTech4 = generate(
    ...
    expect(containsBigInt(friendTech4.context)).toBe(false);
  });
});
