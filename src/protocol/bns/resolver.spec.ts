...
describe('BNS Resolver', () => {
  it('Should not detect as bns resolver', () => {
    const bnsReverse = detect(catchall0xc35c01ac as Transaction);
    expect(bnsReverse).toBe(false);
  });

  it('Should detect as bns resolver transaction', () => {
    const bns1 = detect(bnsSetAddr0x386f74ac as Transaction);
    expect(bns1).toBe(true);
  });

  it('Should detect as bns resolver transaction', () => {
    const bns1 = detect(bnsSetContentHash0xaadbb7bc as Transaction);
    expect(bns1).toBe(true);
  });

  it('Should detect as bns resolver transaction', () => {
    const bns1 = detect(bnsSetRecords0x6565bbb1 as Transaction);
    expect(bns1).toBe(true);
  });

  it('Should generate bns context', () => {
    const bns1 = generate(bnsSetAddr0x386f74ac as Transaction);
  ...
  it('Should generate bns context', () => {
    const bns1 = generate(bnsSetContentHash0xaadbb7bc as Transaction);
    const desc1 = contextSummary(bns1.context);
    expect(desc1).toBe(
  ...
  it('Should generate bns context', () => {
    const bns1 = generate(bnsSetRecords0x6565bbb1 as Transaction);
    const desc1 = contextSummary(bns1.context);
    expect(desc1).toBe(
