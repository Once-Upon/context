...
describe('BNS Name Wrapper', () => {
  ...
  it('Should detect as bns wrapper transaction', () => {
    const bns1 = detect(bnsTransfer0xe2b6affb as Transaction);
    expect(bns1).toBe(true);
  });

  it('Should detect as bns wrapper transaction', () => {
    const bns1 = detect(bnsBatchTransfer0xac234a7b as Transaction);
    expect(bns1).toBe(true);
  });

  it('Should generate bns wrapper context', () => {
    const bns1 = generate(bnsTransfer0xe2b6affb as Transaction);
  ...
  it('Should generate bns wrapper context', () => {
    const bns1 = generate(bnsBatchTransfer0xac234a7b as Transaction);
    const desc1 = contextSummary(bns1.context);
    expect(desc1).toBe(
